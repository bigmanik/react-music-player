    ...
    const filteredSongs =  allSongs.filter((songs)=>{
        const matches =
         songs.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
          songs.artist.toLowerCase().includes(searchQuery.toLowerCase());
        ...
        <div className="playlists">
            ...
            <div className="create-playlist"> 
                ...
                <div className="playlists-list">
                    {playlists.length ===0 ? (<p className="empty-message"> NO playlist created yet </p>)
                     :
                     (playlists.map((playlist, key)=>
                    (<div className="playlist-item" key={key}>
                        <div className="playlist-header">
                            <h3>{playlist.name}</h3>
                            <div className="playlist-actions"> <button className="delete-playlist-btn" onClick={()=> deletePlaylistConformation(playlist)}>Delete</button></div>
                        </div>
                        {/* add song search */}
                        <div className="add-song-section">
                            <div className="search-container">
                                <input type="text"  placeholder="search songs to add..." value={selectedPlaylist?.id === playlist.id ?
                                    searchQuery : ""
                                }  
                                onChange={(e) => {
                        ...
                        <div className="playlist-songs">
                            ...
                            : (
                                 playlist.songs.map((song, key)=> <div key={key} className={`playlist-song ${currentTrackIndex === 
